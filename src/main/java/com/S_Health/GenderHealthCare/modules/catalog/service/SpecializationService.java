...
public class SpecializationService {
    ...
    public SpecializationDTO getSpecializationById(Long id) {
        Specialization specialization = specializationRepository.findById(id)
                .orElseThrow(() -> new ApiException(ErrorCode.NOT_FOUND, CatalogConstants.SPECIALIZATION_NOT_FOUND.formatted(id)));
    ...
    public SpecializationDTO createSpecialization(SpecializationRequest request) {
        ...
        if (specializationRepository.existsByNameAndIsActiveTrue(request.getName().trim())) {
            throw new ApiException(ErrorCode.CONFLICT, CatalogConstants.SPECIALIZATION_NAME_EXISTS);
        }
    ...
    public SpecializationDTO updateSpecialization(Long id, SpecializationRequest request) {
        ...
        if (!specialization.getIsActive()) {
            throw new ApiException(CatalogConstants.SPECIALIZATION_DELETED_UPDATE);
        }
    ...
    public void deleteSpecialization(Long id) {
        ...
        if (!specialization.getIsActive()) {
            throw new ApiException(ErrorCode.CONFLICT, CatalogConstants.SPECIALIZATION_ALREADY_DELETED);
        }

        // Kiểm tra xem chuyên môn có được sử dụng không
        if (!specialization.getServices().isEmpty()) {
            throw new ApiException(ErrorCode.CONFLICT, CatalogConstants.SPECIALIZATION_IN_USE_BY_SERVICES);
        }

        if (!specialization.getConsultants().isEmpty()) {
            throw new ApiException(ErrorCode.CONFLICT, CatalogConstants.SPECIALIZATION_IN_USE_BY_CONSULTANTS);
        }
